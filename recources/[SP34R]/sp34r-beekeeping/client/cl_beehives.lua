        ...
        if inZone > 0 then
            ...
            placeObject("np_beehive", function(pPlaced, pCoords, pHeading)
                if pPlaced then
                    ...
                    local finished = exports["ev-taskbar"]:taskBar(1500, "Installing beehive", false, true, false, false, nil, 5.0, PlayerPedId())
                    ClearPedTasks(PlayerPedId())
                    if finished == 100 then
                        RPC.execute("sp34r-beekeeping:installHive", pCoords, pHeading)
...
AddEventHandler("sp34r-beekeeping:checkBeehive", function(pContext, pEntity)
    local hiveId = getHiveId(pEntity)
    if not hiveId then return end
    showHiveMenu(hiveId)
end)

RegisterUICallback('sp34r-beekeeping:addQueen', function (data, cb)
    ...
    print(data.key.id)
    if finished == 100 then
        RPC.execute("sp34r-beekeeping:addQueen", data.key.id)
        TriggerEvent("inventory:removeItem", "beequeen", 1)
    end
end)

RegisterUICallback('sp34r-beekeeping:removeHive', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    ...
    if finished == 100 then
        ...
        local success = RPC.execute("sp34r-beekeeping:removeHive", data.key, getReady)
        if not success then
...
AddEventHandler("sp34r-beekeeping:harvestHive", function(pContext, pEntity)
    local hiveId = getHiveId(pEntity)
    if not hiveId then return end
    ...
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("sp34r-beekeeping:harvestHive", hiveId)
    end
end)

AddEventHandler("ev-polyzone:enter", function(zone, data)
    if zone == "sp34r-beekeeping:bee_zone" then
        inZone = inZone + 1
        if inZone == 1 then
            local result = RPC.execute("sp34r-beekeeping:getBeehives")
        end
    end
end)
...
RegisterNetEvent('sp34r-beekeeping:trigger_zone')
AddEventHandler('sp34r-beekeeping:trigger_zone', function (type, pData)
    --Update all hives
    if type == 1 then
...
function showHiveMenu(pHiveId)
    ...
    if getStageFromPercent(harvest) < 2 and not hive.has_queen then
        context[#context+1] = {
            title = 'Add Queen bee',
            key = { id = pHiveId },
            action = 'sp34r-beekeeping:addQueen',
    ...
    if harvest >= 95 or myJob == "police" or myJob == "sheriff" or myJob == "state"  then
        context[#context+1] = {
            title = 'Destroy Hive',
            key = { id = pHiveId },
            action = 'sp34r-beekeeping:removeHive',
        }
    end
